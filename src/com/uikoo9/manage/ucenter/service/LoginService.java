...
public class LoginService {
	...
	public QJson login(Map<String, String[]> paras, HttpServletResponse response){
		try {
			...
			if(QStringUtil.allNotEmpty(new String[]{username,password})){
				...
				List<UcenterUserModel> checkUsers = UcenterUserModel.dao.find("select * from t_ucenter_user where ucenter_user_name=? and ucenter_user_password=?", username, password);
	...
	public QJson modifyPwd(Map<String, String[]> paras, HttpServletRequest request){
		try {
			...
			Db.update("update t_ucenter_user set ucenter_user_password=? where id=?", newpwd, id);
